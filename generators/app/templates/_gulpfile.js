...
gulp.task('gen-ts-refs', function () {
	...
	return target.pipe(inject(sources, {
		relative: true,
		starttag: '// inject references start',
		endtag: '// inject references end',
		transform: function (filepath) {
			var end = filepath.substring(filepath.length - 8);
			return end === 'app.d.ts' ? null : '/// <reference path="' + filepath + '" />';
		}
 ...
 * @return {Stream}
 */
gulp.task('wiredep', ['compile-ts', 'templatecache'], function() {
	...
	return gulp
		   ...
		   .pipe(inject(gulp.src(js), {read: false, name: 'application'}))
...
gulp.task('inject', ['wiredep', 'styles', 'templatecache'], function() {
	...
	var cssPath = './build/styles/style.css';
    log('Wire ' + cssPath + ' into ' + config.index);
	
    return gulp
        .src(config.index)
        .pipe(inject(gulp.src(cssPath), { name: 'app',}))
        .pipe(gulp.dest(config.client));
});

/**
 * Add template cache to index. This is separate to other js files
 * so that it doesn't get minified
 * @return {Stream}
 */
gulp.task('inject-templates', ['wiredep', 'templatecache'], function() {
	log('Inject template cache into index');

	var templateCache = config.templateCache.path + '/' + config.templateCache.file;

	return gulp
		   .src(config.index)
		   .pipe(inject(gulp.src(templateCache), {name: 'templates', read: false}))
		   .pipe(gulp.dest(config.client));
});
 ...
 * @return {Stream}
 */
gulp.task('build-specs', ['compile-ts', 'templatecache'], function(done) {
	...
	return gulp
		   ...
		   .pipe(wiredep(options))
		   .pipe(inject(gulp.src(config.js, {read: false}), '', config.jsOrder))
		   .pipe(inject(gulp.src(config.testlibraries, {read: false}), {name: 'testlibraries'}))
		   .pipe(inject(gulp.src(config.specHelpers, {read: false}), {name: 'spechelpers'}))
		   .pipe(inject(gulp.src(specs, {read: false}), {name: 'specs'}))
 ...
 * @return {Stream}
 */
gulp.task('test', ['compile-ts', 'ts-lint', 'templatecache'], function(done) {
 ...
 * --nosync
 */
gulp.task('serve-dev', ['inject', 'inject-templates'], function() {
	serve(true /*isDev*/);
});
